...
class RuneLiteClicker:
    def __init__(self):
        self.previous_screenshot = None
        self.is_watching = False
        self.last_click_time = 0
        self.click_interval = random.randint(1, 10)  # Random interval between 1-10 seconds
    ...
    def click_on_runelite(self, runelite_window):
        """Click on the RuneLite window"""
        try:
            if runelite_window:
                # Get window position and size
                x, y, width, height = runelite_window.left, runelite_window.top, runelite_window.width, runelite_window.height
                
                # Calculate random position within the window (avoid edges)
                click_x = x + random.randint(50, width - 50)
                click_y = y + random.randint(50, height - 50)
                
                # Move mouse and click
                pyautogui.click(click_x, click_y)
                
                timestamp = datetime.now().strftime("%H:%M:%S")
                print(f"[{timestamp}] 🖱️  Clicked on RuneLite at ({click_x}, {click_y})")
            ...
            print(f"Error clicking on RuneLite: {e}")
            return False
    ...
    def start_watching(self):
        """Start the screen watching and clicking process"""
        print("🔍 Starting RuneLite Clicker...")
        print("I'm watching your screen and will click on RuneLite every 1-10 seconds!")
        ...
        while self.is_watching:
            try:
                ...
                if current_frame is not None:
                    ...
                    # Check if it's time to click on RuneLite
                    if current_time - self.last_click_time >= self.click_interval:
                        runelite_window = self.find_runelite_window()
                        if runelite_window:
                            if self.click_on_runelite(runelite_window):
                                # Set new random interval for next click
                                self.click_interval = random.randint(1, 10)
                                self.last_click_time = current_time
                                print(f"   ⏰ Next click in {self.click_interval} seconds")
                        else:
                            print("❌ RuneLite window not found, will retry...")
                            self.last_click_time = current_time  # Reset timer
...
def main():
    """Main function to run the RuneLite Clicker"""
    print("🤖 RuneLite Clicker - AI-Inspired Screen Watcher")
    print("=" * 50)
    ...
    # Create and start the RuneLite Clicker
    clicker = RuneLiteClicker()
