#!/usr/bin/env python3
"""
Safe version of simple template alch - skips crab if position seems wrong
"""
...
class SimpleTemplateAlchSafe:
    def __init__(self):
        ...
        self.max_crab_fails = 5  # Reduced for safety
    ...
    def detect_crab_safe(self, screen):
        """Detect gemstone crab with safety checks"""
        # Use a higher threshold for safety
        position, confidence = self.find_template_in_screen(screen, self.crab_template, threshold=0.6)
        
        if position:
            # Additional safety check - make sure it's in a reasonable area
            x, y = position
            screen_height, screen_width = screen.shape[:2]
            
            # Check if position is in a reasonable area (not too far to the right)
            if x > screen_width * 0.8:  # If it's in the rightmost 20% of screen
                print(f"   ⚠️ Crab detected too far right ({x}, {y}) - skipping for safety")
    ...
    def detect_crab(self, screen):
        """Detect gemstone crab using template matching with safety checks"""
        return self.detect_crab_safe(screen)
    ...
    def start_watching(self):
        """Start the safe version of the alch clicking process"""
        print("🔍 Starting Safe Version - Advanced Anti-Detection Alch Clicker...")
        print("Step 1: Find alchemy spell")
        print("Step 2: Click spell")
        print("Step 3: Find darts")
        print("Step 4: Click darts")
        print("Step 5: Find gemstone crab (with safety checks)")
        ...
        print("🔧 SAFE: Crab detection with position safety checks")
...
def main():
    """Main function to run the Safe Alch Clicker"""
    print("🤖 Safe Version - Advanced Anti-Detection Alch Clicker")
    print("=" * 60)
    ...
    # Create and start the Safe Alch Clicker
    clicker = SimpleTemplateAlchSafe()
